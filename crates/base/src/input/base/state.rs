...
use crate::input::{
    InputExtras as _, Position, RopeExt as _, Selection,
    element::RIGHT_MARGIN,
    layout::LastLayout,
    span::{InputSpan, InputSpanEditMode, SpanPresentation},
};
...
pub struct InputBaseState<M: InputModeKind> {
    ...
    pub(super) presentation: SpanPresentation,
...
impl<M: InputModeKind> InputBaseState<M> {
    ...
    fn new_in_mode(window: &mut Window, cx: &mut Context<Self>) -> Self {
        ...
        Self {
            ...
            spans: Vec::new(),
            presentation: SpanPresentation::new(&Rope::new(), &[]),
    ...
    pub fn default_value(mut self, value: impl Into<SharedString>) -> Self {
        ...
        self.presentation = SpanPresentation::new(&self.text, &self.spans);
...
impl<M: InputModeKind> EntityInputHandler for InputBaseState<M> {
    ...
    fn replace_text_in_range(
        ...
        if had_spans {
            ...
            self.presentation = SpanPresentation::new(&self.text, &[]);
...
mod tests {
    ...
    use anyhow::Result;
    use gpui::{Task, TestAppContext, VisualTestContext};
    ...
    fn moving_back_before_space_reopens_completion_for_the_active_range(cx: &mut TestAppContext) {
        ...
        let input_view = InputView::build_editor(cx, move |mut state| {
            state.extras.lsp.completion_provider = Some(provider);
            state
        });
    ...
    fn rich_spans_preserve_source_and_clamp_editable_integer(cx: &mut TestAppContext) {
        let input_view = InputView::build_textarea(cx, |state| state.auto_grow(1, 4));
        let mut cx = VisualTestContext::from_window(input_view.window_handle.into(), cx);
        let input = input_view.input;
...
impl<M: crate::input::MultiLineMode> InputBaseState<M> {
    ...
    ///
    /// This applies to both ordinary textareas and editor-mode inputs. The
    /// latter is useful for prose composers that need completion providers
    /// without an editor gutter.
