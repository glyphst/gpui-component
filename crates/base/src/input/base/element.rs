...
use gpui::{
    ...
    LayoutId, MouseButton, MouseMoveEvent, MouseUpEvent, PaintQuad, ParentElement as _, Path,
    Pixels, Point, Position, ShapedLine, SharedString, Size, Style, Styled as _, TextAlign,
    TextRun, TextStyle, TransformationMatrix, UnderlineStyle, Window, fill, point, px, quad,
    relative, size,
};
use ropey::Rope;
...
impl<M: InputModeKind> Element for TextElement<M> {
    ...
    fn prepaint(
        ...
        if !state.presentation.spans().is_empty() {
            ...
            let span_styles = state.presentation.spans().iter().filter_map(|span| {
                ...
                (!range.is_empty()).then_some((
                    range,
                    HighlightStyle {
                        color: Some(state.editor_style.span_accent),
                        ..Default::default()
                    },
                ))
            });
            ...
            let editable_styles = state.presentation.spans().iter().filter_map(|span| {
                ...
                (!range.is_empty()).then_some((
                    range,
                    HighlightStyle {
                        color: Some(state.editor_style.foreground),
                        ..Default::default()
                    },
                ))
            });
        ...
        let state = self.state.read(cx);
        let (span_quads, span_editable_quads, span_icons, span_hitboxes) = Self::layout_spans(
            state,
            &last_layout,
            &bounds,
            window,
            state.editor_style.span_accent,
        );
...
mod tests {
    ...
    fn span_fragments_leave_room_above_and_below_the_caret() {
        ...
        assert_eq!(
            TextElement::<InputMode>::inset_span_fragment(fragment, px(2.)),
            Bounds::new(point(px(10.), px(22.)), size(px(80.), px(16.)))
        );
        assert_eq!(
            TextElement::<InputMode>::inset_span_fragment(fragment, px(3.)),
