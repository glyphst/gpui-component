...
impl Transition {
    ...
    fn scaled_duration(&self, factor: f32) -> Duration {
        if factor == 1.0 {
            // `Duration::mul_f32(1.0)` can round a duration up by one nanosecond.
            // At an exact end-time sample that leaves the transition running for
            // one more frame, so preserve the original duration for the identity
            // factor instead of taking the floating-point path.
            self.duration
        } else {
            self.duration.mul_f32(factor)
        }
    }
