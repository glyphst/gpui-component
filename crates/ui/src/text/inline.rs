...
pub(super) struct Inline {
    ...
    selection_target: Option<(Arc<Mutex<InlineState>>, usize)>,
}
...
impl Inline {
    ...
    /// Mirrors this fragment's selection into its source inline state.
    ///
    /// InlineFlow creates one Inline per wrapped fragment. Those fragments
    /// need local text for layout, while Markdown copy traverses the original
    /// source state, so map the fragment-local byte range back into it.
    pub(super) fn selection_target(
        mut self,
        target: Arc<Mutex<InlineState>>,
        source_offset: usize,
    ) -> Self {
        self.selection_target = Some((target, source_offset));
        self
    }
...
impl Element for Inline {
    ...
    fn paint(
        ...
        if let (Some(selection), Some((target, source_offset))) =
            (selection, self.selection_target.as_ref())
            && !selection.is_empty()
            && let Ok(mut target) = target.lock()
        {
            let mapped = Selection::new(
                source_offset + selection.start,
                source_offset + selection.end,
            );
            target.selection = Some(match target.selection {
                Some(existing) => Selection::new(
                    existing.start.min(mapped.start),
                    existing.end.max(mapped.end),
                ),
                None => mapped,
            });
        }
